...
class PyFAT:
    ...
    def load(self, assets_path):
        ...
        r50 = Model.iresnet50()
        r50.load_state_dict(torch.load('./assets/w600k_r50.pth'))
        r50.eval().to(self.device)
        repvgg = Model.create_RepVGG_A0(deploy=False)
        repvgg.load_state_dict(torch.load('./assets/repvgg.pt'))
        ...
        self.model = nn.ModuleList([r50, tf_nas_a, repvgg, mobilenet1, hrnet])
    ...
    def generate(self, im_a, im_v, n):
        ...
        for model in self.model:
            ...
            vic_feats.append(temp)
        print(f'受害者识别：{time.time() - t}')
        ...
        # 定义初始参数
        count = 0
        y_scale_start = 0.8 + n * 0.13
        x_scale_start = 0.8 + n * 0.13
